...
OUT_DIR=latency_perbuffersize_${HOSTNAME}-${REMOTE_HOST}_`date +%Y_%m_%d-%H-%M`
mkdir -p $OUT_DIR
...
for bufsize in 512 1024 2048 4096 8192 16384 32768 65536
do
  echo
  echo ------ Run with buffer size = $bufsize ------
  ...
  sleep 2
  ./rttwr -vmopts "-Dddsi.network.interfaces=dev21726 -Dddsi.network.receiver.udp.buffer.size=${bufsize}" $size 100000 100 > $OUT_DIR/lat_${bufsize}.log
