...
if [ $# -lt 1 ]
then
  echo "Usage : run_throughput_perdatasize_remote.sh <remote_host>:<remote_path>"
  exit
fi
...
OUT_DIR=throughput_reliable_perdatasize_${HOSTNAME}-${REMOTE_HOST}_`date +%Y_%m_%d-%H-%M`
mkdir -p $OUT_DIR
...
do
  ...
  ssh $REMOTE_HOST "cd $REMOTE_DIR;./tprd -bg ${size} > /dev/null 2>/dev/null < /dev/null"
  sleep 2
  ./tpwr $size 30 > $OUT_DIR/tp_${size}.log
